...
class  camera {
    public:
        ...
        int     max_depth = 10; // Maximum number of ray bounces per scene

        double vfov = 90; // Vertcial view angle
        point3 lookfrom = point3(0,0,0);
        point3 lookat   = point3(0,0,-1);
        vec3   vup      = vec3(0,1,0);

        double defocus_angle = 0; // Cariation angle of rays through each pixel
    ...
    private:
        ...
        double pixel_samples_scale; // Color scale factor for a sum of pixel
                                    // samples
        ...
        ray get_ray(int i, int j) const {
            // Construc a camera ray originating from the defocus disc and directed at
